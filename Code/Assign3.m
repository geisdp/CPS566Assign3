...
for i=1:length(images)
    ...
    % verified_matches(1, :) = [1,3,62,160,3.1948];
    if isempty(verified_matches)
        res = img;
    else
        ...
        for k = 1:numMatches
            ...
            x = verified_matches(k, 4);
            y = verified_matches(k, 3);
            ...
            x = max(1, min(x, imgW));
            y = max(1, min(y, imgH));
            boxW = max(0, min(boxW, imgW - x + 1));
            boxH = max(0, min(boxH, imgH - y + 1));
    
            bboxes(k, :) = [x, y, boxW, boxH];
        end
